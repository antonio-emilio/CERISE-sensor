...
// set CS to 0 = active
#define CSACTIVE digitalWrite(19, LOW)
// set CS to 1 = passive
#define CSPASSIVE digitalWrite(19, HIGH)
...
bool Enc28J60Network::init(uint8_t* macaddr)
{
  ...
  initSPI();
  Serial.println("ENC28J60 Network driver for UIP stack");
...
Enc28J60Network::readReg(uint8_t address)
{
  ...
  setBank(address);
  // do the read
  uint8_t retorno = readOp(ENC28J60_READ_CTRL_REG, address);
